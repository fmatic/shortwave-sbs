...
function utcMinutesNow(date = new Date()) {
...
function onAir(item, now = new Date()) {
    ...
    s = timeToMinutes(item.start),
    e = timeToMinutes(item.end);
    if (s === null || e === null)
        return false;
    if (s === e)
        return true;
...
function modeFor(lat, lon, date = new Date()) {
    const e = solarElevation(lat, lon, date);
    return e > 8 ? "Day" : e > -6 ? "Twilight" : "Night"
}
...
function assistantBandScore(band, mode, count, sw) {
    ...
    const kp = Number(sw?.kp || 0),
    sfi = Number(sw?.sfi || 100);
    ...
    if (kp >= 5)
        ...
        score -= 10;
    if (sfi >= 130 && high.includes(band))
        score += 18;
    if (sfi < 90 && low.includes(band))
        score += 8;
...
function dxScore(item, path, aware, sw, date = new Date()) {
    ...
    if (item.country === "CLA")
        score += 20;
    if (["49m", "41m", "31m"].includes(item.band))
        score += 8;
    const kp = Number(sw?.kp || 0),
    sfi = Number(sw?.sfi || 100),
    x = String(sw?.xray || "").toLowerCase();
    const low = ["120m", "90m", "75m", "60m", "49m"],
    mid = ["41m", "31m"],
    high = ["25m", "22m", "19m", "16m", "13m", "11m"];
    if (kp >= 5) {
        score -= 18;
        if (low.includes(item.band))
            score -= 18
    } else if (kp >= 4)
        score -= 8;
    if (sfi >= 150 && high.includes(item.band))
        score += 22;
    else if (sfi >= 120 && high.includes(item.band))
        score += 12;
    if (sfi < 90 && high.includes(item.band))
        score -= 22;
    if ((x.includes("m-class") || x.includes("x-class")) && high.includes(item.band))
        score -= 25;
    if (sfi >= 120 && mid.includes(item.band))
        score += 6;
    ...
    if (se === "transition" && aware.label === "Greyline potential")
        score += 10;
    if (kp >= 5) {
        score -= 20;
        if (high.includes(item.band))
            score -= 15
    }
    if (sfi >= 140 && high.includes(item.band))
        score += 25;
    if (sfi >= 110 && item.band === "31m")
        score += 10;
    if (sfi < 90 && low.includes(item.band))
        score += 10;
    return Math.max(0, Math.round(score));
}
function formatSite(i) {
    return i.txSite || i.txCode || i.type || "—"
}
function opening(band, score) {
    if (score >= 120)
        return `${band} is calling right now.`;
    if (score >= 90)
        return `${band} is worth checking first.`;
    return `${band} is probably your best bet right now.`;
}
function analysis(band, mode, count, sw) {
    const p = [];
    if (mode === "Night")
        p.push("Night-time propagation currently favours the lower shortwave bands.");
    if (mode === "Twilight")
        p.push("Greyline conditions are developing and can produce surprisingly long paths.");
    if (mode === "Day")
        p.push("Daylight propagation is supporting the higher HF bands.");
    p.push(`${count} broadcasts are currently active on ${band}.`);
    const kp = Number(sw?.kp || 0),
    sfi = Number(sw?.sfi || 100);
    if (kp <= 2)
        p.push("Geomagnetic conditions are nice and quiet.");
    if (kp >= 5)
        p.push("Geomagnetic activity may make long paths less predictable.");
    if (sfi >= 140)
        p.push("Solar flux is helping the higher frequencies.");
    return p.join(" ");
}

export async function onRequestGet(context) {
    const requestUrl = new URL(context.request.url);
    const testKey = requestUrl.searchParams.get("test") || "";
    const cacheKey = new Request(
            requestUrl.origin + "/api/bridge?v=3&test=" + encodeURIComponent(testKey), {
            method: "GET"
        });
    ...
    const [schedRes, swRes] = await Promise.all([
                fetch(origin + "/data/schedules.json", {
                    cf: {
                        cacheTtl: 300,
                        cacheEverything: true
                    }
                }),
                fetch(origin + "/data/space-weather.json", {
                    cf: {
                        cacheTtl: 120,
                        cacheEverything: true
                    }
    ...
    for (const c of candidateRows) {
        const s = String(c.item.station || "").trim().toLowerCase(),
        f = String(c.item.freq || "");
        if (stations.has(s) || freqs.has(f))
            continue;
        stations.add(s);
        freqs.add(f);
        selected.push(c);
    ...
    if (guideFreqs.length === 1)
        note = `I'd check ${guideFreqs[0]} kHz first.`;
    if (guideFreqs.length === 2)
        note = `I'd check ${guideFreqs[0]} and ${guideFreqs[1]} kHz first.`;
    if (guideFreqs.length >= 3)
        note = `I'd check ${guideFreqs[0]}, ${guideFreqs[1]} and ${guideFreqs[2]} kHz first.`;
    ...
    const payload = {
        ...
        guide: {
            ...
            mode,
            score: guide.score,
            active_count: guide.active_count,
        ...
        best_dx: selected.map(c => ({
                ...
                compass: c.compass,
                score: c.score,
                path: c.awareness.label
            })),
        space_weather: {
            updated: sw.updated || null,
            kp: sw.kp ?? null,
            sfi: sw.sfi ?? null,
            aIndex: sw.aIndex ?? null,
            xray: sw.xray || null,
            aurora: Boolean(sw.aurora),
            hf: sw.hf || {}
        }
    };
